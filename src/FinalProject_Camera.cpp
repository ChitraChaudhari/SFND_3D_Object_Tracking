...
string FileName = "/home/workspace/SFND_3D_Object_Tracking/results.csv";

//a class to create and write a data in csv file
class CSVWriter
{
	string fileName;
  	string delimeter;
...
{
	std::fstream file;
  	//open the file in truncated mode if first line else in append mode
  	file.open(fileName, std::ios::out | (linesCount ? std::ios::app : std::ios::trunc));
  	//iterate over tthe range and add each element to file seperated by delimeter.
  	for(;first != last;)
    {
    	file<<*first;
      	if(++first != last)
...
int main(int argc, const char *argv[])
{
    ...
    bool bVis = false;            // visualize results
  	...
  	vector<string> header = { "Detector + Descriptor", "ttcLidar", "ttcCamera", "ttcCamera - ttcLidar"};
    ...
    for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex+=imgStepWidth)
    {
        ...
        //continue; // skips directly to the next image without processing what comes beneath
      	...
      	vector<string> dataList; //collects the data needs to store in CSV file
        ...
        string detectorType = "SHITOMASI";
      	dataList.push_back(detectorType);
        ...
        string descriptorType = "BRISK"; // BRISK, BRIEF, ORB, FREAK, AKAZE, SIFT
      	dataList.push_back(descriptorType);
            ...
            for (auto it1 = (dataBuffer.end() - 1)->bbMatches.begin(); it1 != (dataBuffer.end() - 1)->bbMatches.end(); ++it1)
            {
               	...
               	if( currBB->lidarPoints.size()>0 && prevBB->lidarPoints.size()>0 ) // only compute TTC if we have Lidar points
               	{
                    ...
                    computeTTCLidar(prevBB->lidarPoints, currBB->lidarPoints, sensorFrameRate, ttcLidar);
                  	...
                  	dataList.push_back(to_string(ttcLidar));
                    ...
                    computeTTCCamera((dataBuffer.end() - 2)->keypoints, (dataBuffer.end() - 1)->keypoints, currBB->kptMatches, sensorFrameRate, ttcCamera);
                  	cout << "ttcCamera : " << ttcCamera;
                  	dataList.push_back(to_string(ttcCamera));
                    
					dataList.push_back(to_string(ttcCamera - ttcLidar));
                    bVis = true;
                    if (bVis)
                    {
